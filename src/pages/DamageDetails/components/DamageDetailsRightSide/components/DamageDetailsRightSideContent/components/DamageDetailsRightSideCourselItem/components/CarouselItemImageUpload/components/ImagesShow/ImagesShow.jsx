...
const ImagesShow = ({ item, form, setForm}) => {

    const [imageModal, setImageModal] = useState(false)
    const [imageIndex, setImageIndex] = useState(null)

    useEffect(() => {
        if(form.images[item.id] && !form.images[item.id].length) {
            setImageModal(false)
        }
    }, [form])
    ...
    const previous = () => {
        if (imageIndex < 1) {
            setImageIndex(form.images[item.id].length - 1 );
        } else {
    ...
    const next = () => {
        if (imageIndex >= form.images[item.id].length - 1) {
            setImageIndex(0)
        } else {
            setImageIndex(imageIndex + 1);
        }
    ...
    const trash = () => {
        const image = form.images[item.id].filter((item, index) => index !== imageIndex);
        setForm(prevState => ({
            ...prevState,
            images: {
                [item.id]: [...image]
    ...
    return (
        <>
            {
                form.images[item.id] && form.images[item.id].length ?
                    <Stack direction='row' spacing={2}>
                        {
                            form.images[item.id].map((image, idx) => {
                                return <div onClick={() => showImage(idx)} className={style.previewImg} key={idx}> <img src={image} alt="" /> </div>
                            })
                        }
                    </Stack>
                    : null
            ...
            >
                <Box sx={styles} style={{backgroundImage: `url(${form.images[item.id] && form.images[item.id][imageIndex]})`}}>
                    ...
                    <Box className={modalStyle.prevIcon}>
                        <DamageDetailsRightSideSliderButtons icon={prevArrow} iconTitle={'previous'} onClick={previous}/>
                    </Box>

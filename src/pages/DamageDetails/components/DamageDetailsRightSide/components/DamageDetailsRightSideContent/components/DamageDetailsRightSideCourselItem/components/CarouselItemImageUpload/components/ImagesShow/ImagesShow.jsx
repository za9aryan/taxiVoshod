...
const ImagesShow = ({active, images, setImages}) => {

    const [imageModal, setImageModal] = useState(false)
    const [imageIndex, setImageIndex] = useState(null)

    useEffect(() => {
        if(images[active] && !images[active].images.length) {
            setImageModal(false)
        }
    }, [images])
    ...
    const previous = () => {
        if (imageIndex < 1) {
            setImageIndex(images[active].images.length - 1 );
        } else {
    ...
    const next = () => {
        if (imageIndex >= images[active].images.length - 1) {
            setImageIndex(0)
        } else {
            setImageIndex(imageIndex + 1);
        }
    ...
    const trash = () => {
        images[active].images = images[active].images.filter((item, index) => index !== imageIndex);
        setImages(prevState => ({
            ...prevState,
            [active]: {
                images: images[active].images
            }
        }));
        next();
    }
    ...
    return (
        <>
            ...
            >
                <Box sx={styles} style={{backgroundImage: `url(${images[active] && images[active].images[imageIndex]})`}}>

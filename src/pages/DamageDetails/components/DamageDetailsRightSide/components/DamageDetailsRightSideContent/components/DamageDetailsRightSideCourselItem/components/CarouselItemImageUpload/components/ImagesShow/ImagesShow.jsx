...
const ImagesShow = ({ form, setForm, progress, isLoading}) => {
    ...
    const next = () => {
        if (imageIndex >= form.images.length - 1) {
            setImageIndex(0)
        } else {
            setImageIndex(imageIndex + 1);
        }
    ...
    const trash = () => {
        const image = form.images.filter((item, index) => index !== imageIndex);
        setForm(prevState => ({
            ...prevState,
            images: [...image]
        }));
    ...
    return (
        <>
            ...
            >
                <Box sx={styles} style={{backgroundImage: `url(${form.images[imageIndex]?.img.indexOf('data:image') !== -1 ? form.images[imageIndex]?.img : `https://taxivoshod.ru/${form.images[imageIndex]?.img}`})`}}>
